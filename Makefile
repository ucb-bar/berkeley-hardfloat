...
test-$(1)/simv: test-$(1)/Test_$(1).v
	cd test-$(1) && vcs -full64 +define+EXPERIMENT=\"emulator-$(1).vh\" +incdir+../vsrc +rad $$(notdir $$<) ../vsrc/emulator.v -o $$(notdir $$@)

test-$(1)/simv-debug: test-$(1)/Test_$(1).v
	cd test-$(1) && vcs -full64 +define+EXPERIMENT=\"emulator-$(1).vh\" +incdir+../vsrc +define+DEBUG -debug_pp $$(notdir $$<) ../vsrc/emulator.v -o $$(notdir $$@)
