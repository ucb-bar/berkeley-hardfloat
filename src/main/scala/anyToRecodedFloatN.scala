...
{
  def apply(in: UFix, roundingMode: Bits, typeOp: Bits, sigWidth: Int, expWidth: Int, intWidth: Int) = {
    val sign =
    ...
    if (intWidth-sigWidth-1 >= 0)
      ...
      norm_unrounded = Cat(norm_unrounded, Bits(0, sigWidth-intWidth+1))
    norm_unrounded = Cat(Bits(0, 1), norm_unrounded)
    val norm_rounded = Mux(round, norm_unrounded + UFix(1), norm_unrounded)
    ...
    val exponent = Cat(norm_out(intWidth-1), Mux(overflow_unrounded, UFix(1 << (expWidth-2)), exp_rounded(expWidth-2,0)))
    ...
    val exc = Cat(Bits(0,2), overflow, Bits(0,1), inexact)
    (out, exc)
  }
}
