...
| See "docs/divSqrtRecodedFloat64_mulAddZ31.txt" for more details.
*----------------------------------------------------------------------------*/

class divSqrtRecodedFloat64_mulAddZ31 extends Module
{
  val io = new Bundle {
...
//*** NEED TO COMPUTE AS MUCH AS POSSIBLE IN PREVIOUS CYCLE?:
    ...
    val isZeroA_S    = (specialCodeA_S === UInt("b000", 3))
    val isSpecialA_S = (specialCodeA_S(2,1) === UInt("b11", 2))

    val signB_S  = io.b(64)
    val expB_S   = io.b(63,52)
    val fractB_S = io.b(51,0)
    val specialCodeB_S = expB_S(11,9)
    val isZeroB_S    = (specialCodeB_S === UInt("b000", 3))
    val isSpecialB_S = (specialCodeB_S(2,1) === UInt("b11", 2))
    ...
    when ( entering_PA_normalCase ) {
        exp_PA :=
            Mux(io.sqrtOp, expB_S, expA_S + Cat(Fill(3, expB_S(11)), ~expB_S(10,0)))
    ...
    val isZeroA_PA    = (specialCodeA_PA === UInt("b000", 3))
    val isSpecialA_PA = (specialCodeA_PA(2,1) === UInt("b11", 2))
    val sigA_PA = Cat(UInt("b1", 1), fractA_51_PA, fractA_other_PA)

    val isZeroB_PA    = (specialCodeB_PA === UInt("b000", 3))
    val isSpecialB_PA = (specialCodeB_PA(2,1) === UInt("b11", 2))
    val sigB_PA = Cat(UInt("b1", 1), fractB_51_PA, fractB_other_PA)

    val normalCase_PA =
        Mux(sqrtOp_PA, ~isSpecialB_PA & ~isZeroB_PA & ~sign_PA,
    ...
    val isZeroA_PB    = (specialCodeA_PB === UInt("b000", 3))
    val isSpecialA_PB = (specialCodeA_PB(2,1) === UInt("b11", 2))
    val isZeroB_PB    = (specialCodeB_PB === UInt("b000", 3))
    val isSpecialB_PB = (specialCodeB_PB(2,1) === UInt("b11", 2))
    val normalCase_PB =
        Mux(sqrtOp_PB, ~isSpecialB_PB & ~isZeroB_PB & ~sign_PB,
    ...
    val isZeroA_PC    = (specialCodeA_PC === UInt("b000", 3))
    val isSpecialA_PC = (specialCodeA_PC(2,1) === UInt("b11", 2))
    ...
    val isZeroB_PC    = (specialCodeB_PC === UInt("b000", 3))
    val isSpecialB_PC = (specialCodeB_PC(2,1) === UInt("b11", 2))
    ...
    val sigB_PC       = Cat(UInt("b1", 1), fractB_51_PC, fractB_other_PC)

    val normalCase_PC =
        Mux(sqrtOp_PC, ~isSpecialB_PC & ~isZeroB_PC & ~sign_PC,
            ~isSpecialA_PC & ~isSpecialB_PC & ~isZeroA_PC & ~isZeroB_PC)

    val expP2_PC = exp_PC + 2
    val expP1_PC =
        Mux(exp_PC(0), Cat(expP2_PC(13,1), UInt("b0", 1)), Cat(exp_PC(13,1), UInt("b1", 1)))
    ...
    val roundMagUp_PC = Mux(sign_PC, roundingMode_min_PC, roundingMode_max_PC)
    ...
    *------------------------------------------------------------------------*/
    ...
    val cyc_A6_sqrt = (cycleNum_A === 6)
    val cyc_A5_sqrt = (cycleNum_A === 5)
    val cyc_A4_sqrt = (cycleNum_A === 4)

    val cyc_A4_div = entering_PA_normalCase_div

    val cyc_A4 = cyc_A4_sqrt | cyc_A4_div
    val cyc_A3 = (cycleNum_A === 3)
    val cyc_A2 = (cycleNum_A === 2)
    val cyc_A1 = (cycleNum_A === 1)
    ...
    cyc_B9_sqrt  := (cycleNum_B === 9)
    cyc_B8_sqrt  := (cycleNum_B === 8)
    cyc_B7_sqrt  := (cycleNum_B === 7)
    ...
    *------------------------------------------------------------------------*/
    ...
    val zLinPiece_0_A4_div = cyc_A4_div & (fractB_S(51,49) === 0)
    val zLinPiece_1_A4_div = cyc_A4_div & (fractB_S(51,49) === 1)
    val zLinPiece_2_A4_div = cyc_A4_div & (fractB_S(51,49) === 2)
    val zLinPiece_3_A4_div = cyc_A4_div & (fractB_S(51,49) === 3)
    val zLinPiece_4_A4_div = cyc_A4_div & (fractB_S(51,49) === 4)
    val zLinPiece_5_A4_div = cyc_A4_div & (fractB_S(51,49) === 5)
    val zLinPiece_6_A4_div = cyc_A4_div & (fractB_S(51,49) === 6)
    val zLinPiece_7_A4_div = cyc_A4_div & (fractB_S(51,49) === 7)
    val zK1_A4_div =
    ...
    val mulAdd9A_A =
        zFractB_A4_div(48,40) | zK2_A7_sqrt | Mux(~cyc_S, nextMulAdd9A_A, UInt(0))
    val mulAdd9B_A =
        zK1_A4_div | zFractB_A7_sqrt(50,42) | Mux(~cyc_S, nextMulAdd9B_A, UInt(0))
    val mulAdd9C_A =
        zComplK1_A7_sqrt<<10                                                      |
    ...
    val loMulAdd9Out_A = mulAdd9A_A * mulAdd9B_A + Cat(UInt("b0", 1), mulAdd9C_A(17,0))
    val mulAdd9Out_A =
        Cat(Mux(loMulAdd9Out_A(18), mulAdd9C_A(24,18) + 1, mulAdd9C_A(24,18)),
            loMulAdd9Out_A(17,0))

    val zFractR0_A6_sqrt =
        Mux(cyc_A6_sqrt & mulAdd9Out_A(19), ~mulAdd9Out_A>>10, UInt(0))(8,0) // CHISEL
    /*------------------------------------------------------------------------
    | (`sqrR0_A5_sqrt' will usually be >= 1, but not always.)
    *------------------------------------------------------------------------*/
    val sqrR0_A5_sqrt = Mux(exp_PA(0), mulAdd9Out_A<<1, mulAdd9Out_A)
    val zFractR0_A4_div =
        Mux(cyc_A4_div & mulAdd9Out_A(20), ~mulAdd9Out_A>>11, UInt(0))(8,0) // CHISEL
    val zSigma0_A2 = Mux(cyc_A2 & mulAdd9Out_A(11), ~mulAdd9Out_A>>2, UInt(0))(8,0) // CHISEL
    val fractR1_A1 = Mux(sqrtOp_PA, mulAdd9Out_A>>10, mulAdd9Out_A>>9)(14,0) // CHISEL
    val r1_A1 = Cat(UInt("b1", 1), fractR1_A1)
    ...
    when ( cyc_A4_sqrt | cyc_A3 ) {
        partNegSigma0_A := Mux(cyc_A4_sqrt, mulAdd9Out_A, mulAdd9Out_A>>9)(20,0) // CHISEL
    }
    ...
    when ( cyc_A7_sqrt | cyc_A6_sqrt | cyc_A5_sqrt | cyc_A4 | cyc_A2 ) {
        nextMulAdd9B_A :=
            ...
            Mux(cyc_A2,      Cat(UInt("b1", 1), fractR0_A(8,1)), UInt(0))
    }
...
//*** ONLY 30 BITS NEEDED IN CYCLE C6:
    ...
    io.usingMulAdd :=
        Cat(cyc_A4 | cyc_A3_div | cyc_A1_div
             ...
             | cyc_B5_sqrt | cyc_B3_sqrt | cyc_B2_div | cyc_B1_sqrt | cyc_C4,
         cyc_A3 | cyc_A2_div | cyc_B9_sqrt  | cyc_B8_sqrt | cyc_B6 | cyc_B5
             | cyc_B4_sqrt | cyc_B2_sqrt | cyc_B1_div | cyc_C6_sqrt | cyc_C3,
         cyc_A2 | cyc_A1_div | cyc_B8_sqrt  | cyc_B7_sqrt | cyc_B5 | cyc_B4
             | cyc_B3_sqrt | cyc_B1_sqrt | cyc_C5 | cyc_C2,
    ...
    io.mulAddC_2 :=
        ...
        Mux(cyc_E3_sqrt,
            (Mux(exp_PC(0), Cat(sigB_PC(0), UInt("b0", 1)),
                    Cat(sigB_PC(1) ^ sigB_PC(0), sigB_PC(0)))
                    ^ Cat(~extraT_E, UInt("b0", 1))) << 54,
             UInt(0))
    ...
    zComplSigT_C1 :=
        Mux((cyc_C1_div & ~E_C1_div) | cyc_C1_sqrt, ~io.mulAddResult_3(104,51),
            UInt(0)) |
        Mux(cyc_C1_div & E_C1_div, Cat(UInt("b0", 1), ~io.mulAddResult_3(102,50)), UInt(0))
    zComplSigT_C1_sqrt := Mux(cyc_C1_sqrt, ~io.mulAddResult_3(104,51), UInt(0))
    /*------------------------------------------------------------------------
    ...
    *------------------------------------------------------------------------*/
    ...
    when ( cyc_E2 ) {
        ...
        isZeroRemT_E :=
            (remT_E2(53,0) === UInt(0)) & (~sqrtOp_PC | (remT_E2(55,54) === UInt(0)))
    }
    ...
    *------------------------------------------------------------------------*/
    ...
    val posExpX_0001111_E = (posExpX_E(12,6) === UInt("b0001111", 7))
    val posExpX_00011110_E  = posExpX_0001111_E  & ~posExpX_E(5)
    val posExpX_000111100_E = posExpX_00011110_E & ~posExpX_E(4)
    val exp3X_lt_001_E = (sExpX_E(2,0) < UInt("b001", 3))
    val exp3X_lt_010_E = (sExpX_E(2,0) < UInt("b010", 3))
    val exp3X_lt_011_E = (sExpX_E(2,0) < UInt("b011", 3))
    val exp3X_lt_100_E = (sExpX_E(2,0) < UInt("b100", 3))
    val exp3X_lt_101_E = (sExpX_E(2,0) < UInt("b101", 3))
    val exp3X_lt_110_E = (sExpX_E(2,0) < UInt("b110", 3))
    val exp3X_lt_111_E = (sExpX_E(2,0) < UInt("b111", 3))
    val exp5X_00_E = (sExpX_E(4,3) === UInt("b00", 2))
    val exp5X_01_E = (sExpX_E(4,3) === UInt("b01", 2))
    val exp5X_10_E = (sExpX_E(4,3) === UInt("b10", 2))
    val exp5X_lt_00001_E = exp5X_00_E & exp3X_lt_001_E
    val exp5X_lt_00010_E = exp5X_00_E & exp3X_lt_010_E
    val exp5X_lt_00011_E = exp5X_00_E & exp3X_lt_011_E
    val exp5X_lt_00100_E = exp5X_00_E & exp3X_lt_100_E
    val exp5X_lt_00101_E = exp5X_00_E & exp3X_lt_101_E
    val exp5X_lt_00110_E = exp5X_00_E & exp3X_lt_110_E
    val exp5X_lt_00111_E = exp5X_00_E & exp3X_lt_111_E
    val exp5X_lt_01000_E = exp5X_00_E
    val exp5X_lt_01001_E = exp5X_00_E | (exp5X_01_E & exp3X_lt_001_E)
    val exp5X_lt_01010_E = exp5X_00_E | (exp5X_01_E & exp3X_lt_010_E)
    val exp5X_lt_01011_E = exp5X_00_E | (exp5X_01_E & exp3X_lt_011_E)
    val exp5X_lt_01100_E = exp5X_00_E | (exp5X_01_E & exp3X_lt_100_E)
    val exp5X_lt_01101_E = exp5X_00_E | (exp5X_01_E & exp3X_lt_101_E)
    val exp5X_lt_01110_E = exp5X_00_E | (exp5X_01_E & exp3X_lt_110_E)
    val exp5X_lt_01111_E = exp5X_00_E | (exp5X_01_E & exp3X_lt_111_E)
    val exp5X_lt_10000_E = ~sExpX_E(4)
    val exp5X_lt_10001_E = ~sExpX_E(4) | (exp5X_10_E & exp3X_lt_001_E)
    val exp5X_lt_10010_E = ~sExpX_E(4) | (exp5X_10_E & exp3X_lt_010_E)
    val exp5X_lt_10011_E = ~sExpX_E(4) | (exp5X_10_E & exp3X_lt_011_E)
    val exp5X_lt_10100_E = ~sExpX_E(4) | (exp5X_10_E & exp3X_lt_100_E)
    val exp5X_lt_10101_E = ~sExpX_E(4) | (exp5X_10_E & exp3X_lt_101_E)
    val exp5X_lt_10110_E = ~sExpX_E(4) | (exp5X_10_E & exp3X_lt_110_E)
    val exp5X_lt_10111_E = ~sExpX_E(4) | (exp5X_10_E & exp3X_lt_111_E)
    val exp5X_lt_11000_E = (sExpX_E(4,3) != UInt("b11", 2))
    val exp5X_lt_11001_E = exp5X_lt_11000_E | exp3X_lt_001_E
    val exp5X_lt_11010_E = exp5X_lt_11000_E | exp3X_lt_010_E
    val exp5X_lt_11011_E = exp5X_lt_11000_E | exp3X_lt_011_E
    val exp5X_lt_11100_E = exp5X_lt_11000_E | exp3X_lt_100_E
    val exp5X_lt_11101_E = exp5X_lt_11000_E | exp3X_lt_101_E
    val exp5X_lt_11110_E = exp5X_lt_11000_E | exp3X_lt_110_E
    val exp5X_lt_11111_E = exp5X_lt_11000_E | exp3X_lt_111_E
//*** ALL BITS NEEDED?:
    val roundMask_E =
        Cat(posExpX_000111100_E & (sExpX_E(3,0) < UInt("b1110", 4)),
            posExpX_000111100_E & (sExpX_E(3,0) < UInt("b1111", 4)),
            posExpX_000111100_E,
            posExpX_00011110_E & exp5X_lt_10001_E,
            posExpX_00011110_E & exp5X_lt_10010_E,
            posExpX_00011110_E & exp5X_lt_10011_E,
            posExpX_00011110_E & exp5X_lt_10100_E,
            posExpX_00011110_E & exp5X_lt_10101_E,
            posExpX_00011110_E & exp5X_lt_10110_E,
            posExpX_00011110_E & exp5X_lt_10111_E,
            posExpX_00011110_E & exp5X_lt_11000_E,
            posExpX_00011110_E & exp5X_lt_11001_E,
            posExpX_00011110_E & exp5X_lt_11010_E,
            posExpX_00011110_E & exp5X_lt_11011_E,
            posExpX_00011110_E & exp5X_lt_11100_E,
            posExpX_00011110_E & exp5X_lt_11101_E,
            posExpX_00011110_E & exp5X_lt_11110_E,
            posExpX_00011110_E & exp5X_lt_11111_E,
            posExpX_00011110_E,
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_00001_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_00010_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_00011_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_00100_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_00101_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_00110_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_00111_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_01000_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_01001_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_01010_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_01011_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_01100_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_01101_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_01110_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_01111_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_10000_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_10001_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_10010_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_10011_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_10100_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_10101_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_10110_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_10111_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_11000_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_11001_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_11010_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_11011_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_11100_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_11101_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_11110_E),
            posExpX_0001111_E & (~sExpX_E(5) | exp5X_lt_11111_E),
            posExpX_0001111_E,
            (posExpX_E < UInt("b0010000000001", 13)),
            (posExpX_E < UInt("b0010000000010", 13)))
    val incrPosMask_E = ~Cat(UInt("b0", 1), roundMask_E) & Cat(roundMask_E, UInt("b1", 1))
...
//***  SO COULD IMPACT THE CRITICAL PATH AROUND THE BIG MULTIPLIER.)
    ...
    val sigY0_E = sigAdjT_E & Cat(UInt("b1", 1), ~roundMask_E)
    val sigY1_E = (sigAdjT_E | Cat(UInt("b0", 1), roundMask_E)) + 1
    ...
    | X = {T, extraT} + 1.
    *------------------------------------------------------------------------*/
    val trueLtX_E1 = Mux(sqrtOp_PC, ~isNegRemT_E & ~isZeroRemT_E, isNegRemT_E)
    ...
    *------------------------------------------------------------------------*/
    val hiRoundPosBit_E1 =
        hiRoundPosBitT_E ^
            (roundMask_E(0) & ~trueLtX_E1 & all1sHiRoundExtraT_E & extraT_E)
    val anyRoundExtra_E1 = ~trueEqX_E1 | ~extraT_E | ~all1sHiRoundExtraT_E
    val roundEvenMask_E1 =
        Mux(roundingMode_near_even_PC & hiRoundPosBit_E1 & ~anyRoundExtra_E1,
//*** CAN SUBSTITUTE `{roundMask_E, UInt("b1", 1)}'.
    ...
    val sigY_E1 =
        Mux(  (roundMagDown_PC & extraT_E & ~trueLtX_E1 & all1sHiRoundT_E)
         | (roundMagUp_PC
                & ((extraT_E & ~trueLtX_E1 & ~trueEqX_E1) | ~all1sHiRoundT_E))
    ...
    val sExpY_E1 =
        ...
        Mux( sigY_E1(53) &  sqrtOp_PC,            (expP2_PC>>1) + UInt("h400", 12), UInt(0))
...
//*** COMPARE WITH MULTIPLIER UNIT:
    ...
    val underflowY_E1 =
        totalUnderflowY_E1 | ((posExpX_E <= UInt("b0010000000001", 13)) & inexactY_E1)
...
//***  `underflow_E1'?
    ...
    val expOut_E1 =
        (expY_E1
             ...
             & ~Mux(notNaN_isInfOut_E1,      ~UInt("b110111111111", 12), UInt(0))) |
            Mux(pegMinFiniteMagOut_E1, UInt("b001111001110", 12), UInt(0))         |
            Mux(pegMaxFiniteMagOut_E1, UInt("b101111111111", 12), UInt(0))         |
            Mux(notNaN_isInfOut_E1,    UInt("b110000000000", 12), UInt(0))         |
            Mux(isNaNOut_PC,           UInt("b111000000000", 12), UInt(0))
    val fractOut_E1 =
        Mux(totalUnderflowY_E1 & roundMagUp_PC, UInt(0), fractY_E1) |
            Mux(isNaNOut_PC | pegMaxFiniteMagOut_E1, UInt("hFFFFFFFFFFFFF", 52), UInt(0))
